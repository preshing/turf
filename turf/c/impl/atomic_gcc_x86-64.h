...
TURF_C_INLINE uint32_t turf_compareExchange32Relaxed(turf_atomic32_t* object, uint32_t expected, uint32_t desired) {
    ...
    // "=a"(previous) means the asm block outputs EAX to previous, because CMPXCHG puts the old value in EAX.
    // "+m"(object->nonatomic) is the memory address that is read/written. This becomes %1.
    // "q"(desired) puts desired into any of EBX, ECX or EDX before the block. This becomes %2.
    // "0"(expected) puts expected in the same register as "=a"(previous), which is EAX, before the block.
...
TURF_C_INLINE uint32_t turf_fetchAdd32Relaxed(turf_atomic32_t* object, int32_t operand) {
    // "=r"(previous) chooses any general register, makes that %0, and outputs this register to previous after the
    // block.
    // "+m"(object->nonatomic) is the memory address that is read/written. This becomes %1.
    // "0"(operand) puts operand into the same register as %0 before the block.
    // volatile is required. Otherwise, if the return value (previous) is unused, the asm block
...
TURF_C_INLINE uint64_t turf_load64Relaxed(const turf_atomic64_t* object) {
    ...
    // Essentially, we perform turf_compareExchange64Relaxed(object, _dummyValue, _dummyValue).
    // "=&A"(previous) outputs EAX:EDX to previous after the block, while telling the compiler that
    // these registers are clobbered before %1 is used, so don't use EAX or EDX for %1.
    // "m"(object->nonatomic) loads object's address into a register, which becomes %1, before the block.
...
TURF_C_INLINE void turf_store64Relaxed(turf_atomic64_t* object, uint64_t desired) {
    ...
    // "=m"(object->nonatomic) loads object's address into a register, which becomes %0, before the block,
    ...
    // "A"(expected) loads the previous value of object->nonatomic into EAX:EDX before the block.
...
TURF_C_INLINE uint64_t turf_compareExchange64Relaxed(turf_atomic64_t* object, uint64_t expected, uint64_t desired) {
    ...
    // "+m"(object->nonatomic) is the memory address that is read/written. This becomes %1.
    // "b" and "c" move desired to ECX:EBX before the block.
    // "0"(expected) puts expected in the same registers as "=a"(previous), which are EAX:EDX, before the block.
    uint64_t previous;
